...
- `web/`: the new full-screen SPA/PWA frontend (Vite + TanStack Router + Apollo + HeroUI 3). GraphQL documents are generated per entity by `web/scripts/generate-entity-documents.mjs`; see `web/README.md` and `web/STYLE_GUIDE.md`. Serves the live site on :3000 (`web/live.sh start|stop|restart|status`, or `make live-web`); the old `frontend/` app is retired and must not be started on :3000. `DEV_SERVER_PUBLIC_URL` in `web/.env.local` sets the public origin for HMR and allowed hosts.
