...
export function useInfiniteScroll(options: UseInfiniteScrollOptions) {
  ...
  const findScrollParent = useCallback((element: Element | null) => {
  ...
  useEffect(() => {
    ...
    const root = findScrollParent(sentinelRef.current);
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (
          ...
          dataLengthRef.current >= totalCountRef.current
        ) {
          return;
        }
        ...
        if (import.meta.env.DEV)
          console.log(
            `[InfiniteScroll] Loading more... (dataLength: ${dataLengthRef.current}, attempts: ${loadMoreAttempts.current})`
          );
  ...
  }, [findScrollParent, onLoadMore, threshold]);
  ...
  useEffect(() => {
    ...
    const needsRebuild = root !== observerRootRef.current;

    if (needsRebuild && observer) {
      observer.disconnect();
      observerRef.current = null;
    }

    if (!observerRef.current) {
      const rebuiltObserver = new IntersectionObserver(
        (entries) => {
          const entry = entries[0];
          if (!entry.isIntersecting) return;
          ...
          const now = Date.now();
          const timeSinceLastLoad = now - lastLoadTime.current;
          if (timeSinceLastLoad < LOAD_COOLDOWN_MS) return;
          if (dataLengthRef.current === 0 && loadMoreAttempts.current >= 3) {
            return;
          }
      ...
      observerRef.current = rebuiltObserver;
      observerRootRef.current = root;
    }
  ...
  }); // Run on every render to catch sentinel/root changes
