...
import { type Episode } from "../lib/graphql";
...
export function PersistentPlayer() {
  ...
  const isVideoSession = contentType === "EPISODE" || contentType === "MOVIE";
  const isCastingThisMedia = Boolean(
    castSession?.mediaFileId &&
    session?.mediaFileId &&
    castSession.mediaFileId === session.mediaFileId,
  );
  ...
  }, [castStop, currentTime, duration, isCastingThisMedia, stopPlayback]);
  ...
  const displayCurrentTime = isCastingThisMedia
    ? castSession?.currentTime || 0
    : currentTime;
  const displayDuration = isCastingThisMedia
    ? castSession?.duration || 0
    : duration;
  const progress =
  ...
  return (
    <>
      ...
      <div
        ...
        onMouseLeave={
          isExpanded && !isPaused ? () => setShowControls(false) : undefined
        }
      >
                    ...
                    <div className="flex items-center gap-2 text-white">
                      ...
                      <div className="flex items-center">
                        ...
                        <VolumeControl
                          volume={
                            isCastingThisMedia
                              ? castSession?.volume || 0
                              : volume
                          }
                          isMuted={
                            isCastingThisMedia
                              ? castSession?.isMuted || false
                              : isMuted
                          }
          ...
          <div className="flex items-center justify-between px-2 py-1.5 bg-default-100 rounded-b-lg">
            <div className="flex items-center">
              ...
              <VolumeControl
                ...
                isMuted={
                  isCastingThisMedia ? castSession?.isMuted || false : isMuted
                }
                onVolumeChange={handleVolumeChange}
                onMuteToggle={toggleMute}
                size="sm"
