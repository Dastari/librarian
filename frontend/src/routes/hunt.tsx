...
function SearchPage() {
  ...
  const [sortColumn, setSortColumn] = useQueryState(
    "sort",
    parseAsString.withDefault("seeders"),
  );
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      addToast({
        title: "Search Error",
        description:
          err instanceof Error ? err.message : "Failed to search indexers",
        color: "danger",
      });
    } finally {
            ...
            <div className="flex items-center gap-2 text-xs text-default-400 mt-0.5">
              ...
              {release.magnetUri || release.link ? (
                <Chip
                  ...
                  variant="flat"
                  color="primary"
                  className="h-4 text-[10px] flex items-center"
                >
      ...
      <Card>
        <CardBody className="flex flex-row items-center gap-4">
          <Select
            ...
            className="w-40"
            classNames={{
              mainWrapper: "rounded-xl overflow-hidden",
            }}
            size="sm"
          >
        ...
        <div className="flex items-center gap-4 text-sm text-default-500">
          <span>
            ...
            <strong className="text-foreground">
              {searchMeta.totalReleases}
            </strong>{" "}
            releases from {searchMeta.indexerCount} indexer
            {searchMeta.indexerCount !== 1 ? "s" : ""}
          </span>
          <span>
          ...
          {searchMeta.errors.length > 0 && (
            <Tooltip content={searchMeta.errors.join("\n")}>
              <Chip size="sm" color="warning" variant="flat">
                {searchMeta.errors.length} error
                {searchMeta.errors.length !== 1 ? "s" : ""}
              </Chip>
            </Tooltip>
          )}
        </div>
      ...
      {isSearching ? (
        ...
        <Card className="bg-content1/50 border-default-300 border-dashed border-2">
          <CardBody className="py-12 text-center">
            ...
            <p className="text-default-500">
              Enter a search term above to find releases across all your
              configured indexers.
            </p>
          </CardBody>
        </Card>
