...
impl<'a> CastDevice<'a> {
    ...
    /// Connect without host verification while applying OS-level transport
    /// deadlines. This is Librarian's minimal hardening extension to the
    /// pinned 0.21.0 crate; upstream's constructor uses an unbounded
    /// `TcpStream::connect` and does not expose the socket.
    pub fn connect_without_host_verification_with_timeouts<S>(
        host: S,
        port: u16,
    ...
    {
        let host = host.into();
        let socket_address = (host.as_ref(), port)
            .to_socket_addrs()?
            .next()
            .ok_or_else(|| {
                std::io::Error::new(
                    std::io::ErrorKind::AddrNotAvailable,
                    "Cast destination did not resolve to a socket address",
                )
            })?;
