...
use crate::db::{Database, LibraryRecord, MovieRecord, TvShowRecord};
use crate::services::graphql::entities::{
    LibraryEntity, MovieEntity, AlbumEntity, AudiobookEntity, ArtistEntity, TrackEntity,
    ArtistEntityWhereInput, TrackEntityWhereInput,
};
use crate::services::graphql::filters::StringFilter;
use crate::services::graphql::orm::EntityQuery;
...
impl EffectiveQualitySettings {
    ...
    /// Get quality settings for a movie using entity types (new API)
    pub fn from_library_and_movie_entity(library: &LibraryEntity, _movie: &MovieEntity) -> Self {
        // Movies no longer have quality overrides - use library settings
        Self {
            allowed_resolutions: library.allowed_resolutions.clone(),
            allowed_video_codecs: library.allowed_video_codecs.clone(),
...
/// Hunt for a specific movie using entity types (new API)
pub async fn hunt_single_movie_entity(
    db: &Database,
    movie: &MovieEntity,
    ...
    info!(
        ...
        movie_title = %movie.title,
        movie_year = ?movie.year,
        "Hunting for specific movie (entity)"
    );

    let mut result = HuntResult::default();
    result.searched = 1;

    let quality_settings = EffectiveQualitySettings::from_library_and_movie_entity(library, movie);
    ...
    info!(
        ...
        movie_title = %movie.title,
        release_count = all_releases.len(),
        "Processing releases for movie"
    );

    // Find best release
    let best_release = select_best_movie_release(&all_releases, &quality_settings);
    ...
    match best_release {
        Some(release) => {
            info!(
                ...
                movie_title = %movie.title,
                release_title = %release.title,
                "Found suitable release, downloading"
            );

            let user_id = Uuid::parse_str(&movie.user_id).ok();
            match download_release(&release, torrent_service, indexer_manager, user_id).await {
                Ok(_) => {
                    result.downloaded = 1;
                    info!(
                        job = "auto_hunt",
                        movie_title = %movie.title,
                        release_title = %release.title,
                        "Successfully downloaded movie release"
                    );
                }
                Err(e) => {
                    error!(
                        ...
                        release_title = %release.title,
                        error = %e,
                        "Failed to download movie release"
                    );
                    result.failed = 1;
                }
            }
...
/// Hunt for a specific album using entity types
pub async fn hunt_single_album_entity(
    db: &Database,
    album: &AlbumEntity,
    library: &LibraryEntity,
    torrent_service: &Arc<TorrentService>,
    ...
    info!(
        ...
        album_id = %album.id,
        album_name = %album.name,
        "Hunting for specific album (entity)"
    );

    let mut result = HuntResult::default();
    result.searched = 1;

    // Get artist name using entity query
    let artist_name = EntityQuery::<ArtistEntity>::new()
        .filter(&ArtistEntityWhereInput {
            id: Some(StringFilter::eq(&album.artist_id)),
            ..Default::default()
        })
        .fetch_one(db.pool())
        .await
    ...
    // Get expected tracks for this album using entity query
    let expected_tracks = EntityQuery::<TrackEntity>::new()
        .filter(&TrackEntityWhereInput {
            album_id: Some(StringFilter::eq(&album.id)),
            ..Default::default()
        })
        .fetch_all(db.pool())
        .await
    ...
    info!(
        ...
        "Searching indexers for album (entity) with query: '{}'",
        search_term
    );
    ...
    if all_releases.is_empty() {
        info!(
            ...
            album_name = %album.name,
            artist = %artist_name,
            "No releases found for album across all indexers"
        );
        result.skipped = 1;
        return Ok(result);
    }
    ...
    // Separate releases with .torrent files from magnet-only
    ...
    // Convert expected_tracks from entity to the format expected by match_tracks
    // TrackEntity fields: track_number, title, disc_number
    let expected_tracks_converted: Vec<crate::db::TrackRecord> = expected_tracks
        .iter()
        .map(|t| crate::db::TrackRecord {
            id: Uuid::parse_str(&t.id).unwrap_or_default(),
            album_id: Uuid::parse_str(&t.album_id).unwrap_or_default(),
            title: t.title.clone(),
            track_number: t.track_number,
            disc_number: t.disc_number.unwrap_or(1),
            duration_secs: t.duration_secs,
            media_file_id: t.media_file_id.as_ref().and_then(|s| Uuid::parse_str(s).ok()),
            download_status: t.download_status.clone().unwrap_or_else(|| "missing".to_string()),
            musicbrainz_id: t.musicbrainz_id.clone(),
            isrc: t.isrc.clone(),
    ...
    if has_track_info {
        for (release, _score) in &torrent_releases {
                ...
                Ok(torrent_bytes) => {
                    match parse_torrent_files(&torrent_bytes) {
                        Ok(files) => {
                            ...
                            // Match tracks
                            let match_result = match_tracks(&expected_tracks_converted, &files);
                            ...
                            info!(
                                ...
                                expected = expected_tracks_converted.len(),
                                "Track matching result"
                            );

                            // Accept if most tracks match (at least 75%)
                            let match_threshold = (expected_tracks_converted.len() as f32 * 0.75) as usize;
                            if match_result.matched_count >= match_threshold {
                                let user_id = Uuid::parse_str(&library.user_id).ok();
                                match download_release(release, torrent_service, indexer_manager, user_id).await {
                                    Ok(_) => {
                                        result.downloaded = 1;
                                        downloaded = true;
                                        info!(
                                            ...
                                            album_name = %album.name,
                                            release_title = %release.title,
                                            "Successfully downloaded album release"
                                        );
                                        break;
                                    }
                                    Err(e) => {
                                        error!(
                                            ...
                                            release_title = %release.title,
                                            error = %e,
                                            "Failed to download album release"
                                        );
                                    }
                                }
                            }
    ...
    // Fall back to magnet releases if no torrent was downloaded
    if !downloaded && !magnet_releases.is_empty() {
        // Take the highest scored magnet release
        if let Some((release, _score)) = magnet_releases.first() {
            let user_id = Uuid::parse_str(&library.user_id).ok();
            match download_release(release, torrent_service, indexer_manager, user_id).await {
                Ok(_) => {
                    result.downloaded = 1;
                    info!(
                        ...
                        album_name = %album.name,
                        release_title = %release.title,
                        "Downloaded album via magnet (no track validation)"
                    );
                }
                Err(e) => {
                    error!(
                        ...
                        release_title = %release.title,
                        error = %e,
                        "Failed to download album magnet"
                    );
                    result.failed = 1;
                }
            }
...
/// Hunt for a specific audiobook using entity types
pub async fn hunt_single_audiobook_entity(
    db: &Database,
    ...
    info!(
        ...
        audiobook_title = %audiobook.title,
        "Hunting for specific audiobook (entity)"
    );

    let mut result = HuntResult::default();
    result.searched = 1;

    // Build search query - search for audiobook by title and author if available
    let search_term = audiobook.title.clone();

    let mut query = TorznabQuery::book_search(&search_term);
    query.categories = categories::audiobooks();

    info!(
        job = "auto_hunt",
        audiobook_title = %audiobook.title,
        "Searching indexers for audiobook (entity)"
    );
    ...
    // For audiobooks, prefer releases that match the title closely
    // Score releases by title similarity
    let mut scored_releases: Vec<(&ReleaseInfo, f32)> = all_releases
        .iter()
        .map(|r| {
            let title_lower = r.title.to_lowercase();
            let audiobook_title_lower = audiobook.title.to_lowercase();
            
            // Simple scoring based on title containment
            let score = if title_lower.contains(&audiobook_title_lower) {
                1.0
            } else if audiobook_title_lower.split_whitespace().all(|word| title_lower.contains(word)) {
                0.8
            } else {
                0.5
            };
    ...
    for (release, _score) in scored_releases {
        let user_id = Uuid::parse_str(&library.user_id).ok();
        match download_release(release, torrent_service, indexer_manager, user_id).await {
            Ok(_) => {
                result.downloaded = 1;
                info!(
                    ...
                    audiobook_title = %audiobook.title,
                    release_title = %release.title,
                    "Successfully downloaded audiobook release"
                );
                break;
            }
            Err(e) => {
...
async fn hunt_tv_episodes_impl(
    ...
    for show in shows {
        // Skip if monitor_type is NONE
        if show.monitor_type.to_uppercase() == "NONE" {
            debug!(
                job = "auto_hunt",
                show_name = %show.name,
                "Show monitor_type is NONE, skipping"
            );
            continue;
        }

        // Get missing episodes for this show, respecting monitor_type
        // - ALL: Get all episodes without files that have aired
        // - FUTURE: Get only episodes that aired after the show was added
        let episodes: Vec<(String, i32, i32)> = if show.monitor_type.to_uppercase() == "FUTURE" {
            // FUTURE: Only episodes that aired after the show was added to the library
            sqlx::query_as(
                r#"
            ...
            .bind(show.created_at.format("%Y-%m-%d").to_string())
            .bind(MAX_HUNT_PER_RUN as i32)
            .fetch_all(db.pool())
            .await?
        } else {
