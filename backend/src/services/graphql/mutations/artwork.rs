...
impl ArtworkMutations {
    ...
    async fn recache_movie_artwork(&self, ctx: &Context<'_>, #[graphql(name = "MovieId")] movie_id: String) -> Result<bool> {
        let db = ctx.data_unchecked::<Database>();
        
        // Fetch movie details
        let movie: Option<(String, String)> = sqlx::query_as(
            "SELECT poster_url, backdrop_url FROM movies WHERE id = ?1"
        )
        .bind(&movie_id)
        .fetch_optional(db)
        .await?;
