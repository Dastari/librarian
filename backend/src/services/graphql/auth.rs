...
/// Verify a JWT token and extract user info using the given secret.
/// The secret must be loaded from the database (auth service); never from environment or GraphQL.
pub fn verify_token(token: &str, jwt_secret: &str) -> Result<AuthUser> {
    let jwt_secret = jwt_secret.trim();
    if jwt_secret.is_empty() {
        return Err(async_graphql::Error::new("JWT secret not configured"));
    }
