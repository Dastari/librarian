...
//!
//! This library crate holds the whole backend; `src/main.rs` is a thin
//! wrapper around [`run`]. Exposing a lib target lets `backend/tests/*`
//! boot the real service stack instead of re-implementing it.
...
/// Build the Tokio runtime and run the backend to completion.
