...
#[cfg(feature = "postgres")]
impl<'r> sqlx::FromRow<'r, sqlx::postgres::PgRow> for TvShowRecord {
    fn from_row(row: &'r sqlx::postgres::PgRow) -> std::result::Result<Self, sqlx::Error> {
        ...
        Ok(TvShowRecord {
            ...
            auto_download_override: row.try_get("auto_download_override")?,
            backfill_existing: row.try_get("backfill_existing")?,
            organize_files_override: row.try_get("organize_files_override")?,
            rename_style_override: row.try_get("rename_style_override")?,
            auto_hunt_override: row.try_get("auto_hunt_override")?,
            episode_count: row.try_get("episode_count")?,
            episode_file_count: row.try_get("episode_file_count")?,
            size_bytes: row.try_get("size_bytes")?,
            created_at: row.try_get("created_at")?,
            updated_at: row.try_get("updated_at")?,
            allowed_resolutions_override: row.try_get("allowed_resolutions_override")?,
            allowed_video_codecs_override: row.try_get("allowed_video_codecs_override")?,
            allowed_audio_formats_override: row.try_get("allowed_audio_formats_override")?,
            require_hdr_override: row.try_get("require_hdr_override")?,
            allowed_hdr_types_override: row.try_get("allowed_hdr_types_override")?,
            allowed_sources_override: row.try_get("allowed_sources_override")?,
            release_group_blacklist_override: row.try_get("release_group_blacklist_override")?,
            release_group_whitelist_override: row.try_get("release_group_whitelist_override")?,
        })
    }
}
