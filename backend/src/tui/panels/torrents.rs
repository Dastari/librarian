...
pub fn spawn_torrent_updater(torrent_service: Arc<TorrentService>, stats: SharedTorrentList) {
    tokio::spawn(async move {
        loop {
            let mut list = match torrent_service.list_torrents().await {
                Ok(list) => list,
                Err(_) => {
...
impl Panel for TorrentsPanel {
    ...
    fn render(&self, frame: &mut Frame, area: Rect, focused: bool) {
        ...
        let border_style = if focused {
            Theme::border(PanelKind::Torrents)
        } else {
            Theme::border_dim()
        };
        ...
        let graph_height = if show_graph && inner.height > 12 {
            8
        } else if show_graph && inner.height > 8 {
            6
        } else {
            0
        };
        ...
        let chunks = if graph_height > 0 {
            Layout::vertical([Constraint::Length(graph_height), Constraint::Min(3)]).split(inner)
        } else {
        ...
        let list_area = if graph_height > 0 {
            chunks[1]
        } else {
            chunks[0]
        };
        ...
        if torrents.is_empty() {
            if list_area.height > 0 && list_area.width > 15 {
                ...
                frame.render_widget(
                    Paragraph::new(Span::styled("No active torrents", Theme::dim())),
                    text_area,
                );
            }
            return;
...
fn render_graph_with_stats(
    ...
    let stats_width = 22;
    let chunks =
        Layout::horizontal([Constraint::Min(20), Constraint::Length(stats_width)]).split(area);
    ...
    let down_points: Vec<(f64, f64)> = stats
        .download_history
        .iter()
        .enumerate()
        .rev()
        ...
        .map(|(i, &rate)| {
            let x = (graph_width
                .saturating_sub((stats.download_history.len().saturating_sub(i)) / 2))
                as f64;
    ...
    let up_points: Vec<(f64, f64)> = stats
        .upload_history
        .iter()
        .enumerate()
        .rev()
        .take(graph_width * 2)
        .map(|(i, &rate)| {
            let x = (graph_width.saturating_sub((stats.upload_history.len().saturating_sub(i)) / 2))
                as f64;
            let y =
                (area.height as f64 / 2.0) - (rate / max_up).min(1.0) * (area.height as f64 / 2.0);
            (x, y)
        })
        .collect();
